...
(defcustom polymode-eval-function-defaults-list
  '((r-mode . ess-eval-region)
    (haskell-mode . haskell-eva))
  "Default evaluation functions to use for each major mode")

(defcustom polymode-get-inferior-process-alist
  '((r-mode . ess-get-process)
    (R-mode . ess-get-process)))


(defun pm--eval-string (eval-fun string)
  (funcall eval-fun string))

(defgeneric pm-eval-string (chunkmode eval-string))


(defmethod pm-eval-string ((chunkmode pm-chunkmode) &optional type)

  (pm--eval-string (oref pm/chunkmode mode) eval-string))

(defmethod pm-eval-string ((chunkmode pm-hbtchunkmode) &optional type)

  )

(defun poly-eval-chunk (&optional vis)
  "Send the current Rmd chunk to inferior process."
  (interactive "P")
  (pm-execute-narrowed-to-span
   'poly-eval-region (point-min) (point-max) vis "Eval chunk"))

(defun poly-eval-region (beg end vis types)
  "Send all chunks of TYPES in region BEG to END to inferior process."
  (interactive "r\nP")
  (pm-map-over-body-spans-same-type types
                                    (lambda ()
                                      (poly-eval-chunk vis))
                                    beg end))

(defun poly-eval-buffer (vis)
  "Send all chunks in the buffer to inferior process."
  (interactive "P")
  (poly-eval-region (point-min) (point-max) vis ))

(defun poly-eval-buffer-from-beg-to-here (vis)
  "Send all chunks from the start of the buffer to current
point to inferior process."
  (interactive "P")
  (ess-Rmd-eval-region (point-min) (point) vis))

(defun poly-eval-buffer-from-here-to-end (vis)
  "Send all chunks from current point to the end of the buffer to inferior process."
  (interactive "P")
