...
(defun polymode-select-indirect-buffer ()
  ...
  (condition-case error
      (let ((span (pm/get-inermost-span)))
        (if (car span)
          ...
          ;; nil, means the base buffer
          (pm--select-buffer (pm/base-buffer)))
        (error
         (message "%s" (error-message-string err))))))

(defun pm--select-buffer (buff)
...
;;VS[26-08-2012]: Dave Love's comment:
...
point."
  (let ((smodes (oref pm/config :submodes))
	(start (point-min))
	(end (point-max))
	(pos (or pos (point)))
        submode span val)
...
(defun pm/narrow-to-span ()
  ...
  (unless (= (point-min) (point-max))
    (let ((span (pm/get-inermost-span)))
      (if span 
          (apply #'narrow-to-region (cdr span))
        (apply #'narrow-to-region
               (cdr (pm/get-span (oref pm/config :base-submode)))
               )))))
...
Return newlly created buffer."
  ...
  (with-current-buffer (pm/base-buffer)
    ...
    (with-current-buffer new-buffer
      (let ((pm/mode-p t)) ;;major-modes might check it 
        (funcall mode))
      ;; Now we can make it local:
      (set (make-local-variable 'pm/mode-p) t)
      ...
      (add-hook 'post-command-hook 'polymode-select-indirect-buffer nil t)
      ...
      (run-hooks 'pm/indirect-buffer-hook))
    new-buffer))
